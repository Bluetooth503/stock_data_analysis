...
def download_batch_stock_data(stocks: list, date: str) -> None:
    ...
    try:
        ...
        def download_callback(data):
            nonlocal download_completed, last_progress, total_stocks
            current_progress = data['finished']
            total_stocks = data['total']
            
            if current_progress // 500 > last_progress // 500:
                logger.info(f"已完成:{current_progress}/{total_stocks} - {data['message']}")
                last_progress = current_progress
            
            # 当finished等于total时，表示下载完成
            if current_progress == total_stocks:
                download_completed = True
                logger.info(f"数据下载完成，共处理{total_stocks}只股票")
        ...
        while not download_completed:
            if time.time() - start_time > timeout:
                raise TimeoutError(f"下载数据超时（{timeout}秒）")
            time.sleep(1)  # 每秒检查一次下载状态
        ...
        logger.error(f"处理 {stocks} 的数据时出错: {str(e)}")
